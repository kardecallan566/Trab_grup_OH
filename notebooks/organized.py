...
class CriteriosDeParagem:
  ...
  def firstElite(self, elite: float):
...
def algoritmo_genetico(
  ...
  populacao = sorted(populacaoInicial(tamanho_populacao, log_level=log_level+1), key=lambda c: aptidao(c))
  ...
  while not criterios_de_paragem.shouldStop():
    ...
    new_populacao = sorted(new_populacao, key=lambda c: aptidao(c), reverse=True)
